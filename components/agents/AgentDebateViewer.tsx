...
// Single message bubble component - compact version
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        {/* Header - only show for first message in a sequence, more compact */}
        {!isConsecutive && (
          <div className="flex items-center gap-1.5 mb-0.5">
            <span className={cn("font-medium text-xs", tierStyle.text)}>
        ...
        <div className={cn(
          "rounded-xl px-2.5 py-1.5 max-w-[95%] inline-block",
          ...
          <div className="flex items-start gap-1.5">
            <svg 
              className={cn("w-3.5 h-3.5 mt-0.5 flex-shrink-0", typeStyle.color)} 
              fill="none" 
              viewBox="0 0 24 24" 
              stroke="currentColor" 
              strokeWidth={2}
...
// Typing indicator component - compact version
  ...
  return (
    <div className="flex gap-2 mt-2 animate-in fade-in duration-200">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-muted-foreground">is thinking...</span>
      </div>
    </div>
  );
}
    ...
    <Card className="flex flex-col overflow-hidden">
      <CardHeader className="py-2 px-3 flex-shrink-0 border-b border-border/50">
        <div className="flex items-center justify-between gap-2">
          ...
          {isLive && (
            <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-5 bg-green-500/10 text-green-500 border-green-500/30">
              Live
            </Badge>
          )}
        </div>
      ...
      <CardContent className="flex-1 overflow-hidden p-0">
        {hasMessages || typingAgent ? (
          <div 
            ref={scrollRef}
            className="max-h-[320px] overflow-y-auto px-3 py-2 scroll-smooth"
          >
          ...
          <div className="py-6 px-4 flex flex-col items-center justify-center text-muted-foreground">
            <div className="relative mb-3">
              {/* Animated agent avatars - smaller */}
              <div className="flex -space-x-2">
                {["intake", "research", "strategy", "intelligence"].map((tier, i) => (
                  <div
                    key={tier}
                    className={cn(
                      "w-8 h-8 rounded-full flex items-center justify-center border border-background",
                      TIER_COLORS[tier as Tier].bg,
                      "animate-pulse"
                    )}
                    style={{ animationDelay: `${i * 200}ms` }}
                  >
                    <span className={cn("text-sm", TIER_COLORS[tier as Tier].text)}>
            ...
            <p className="font-medium text-sm mb-1">Agents Standing By</p>
            <p className="text-xs text-center max-w-[200px]">
              Start analysis to watch agents collaborate
            </p>
